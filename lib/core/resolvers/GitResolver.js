...
GitResolver.prototype._findResolution = function (target) {
    ...
    if (semver.validRange(target)) {
        ...
        .then(function (versions) {
            ...
            //acc customization
            //check if project repo name matches pattern           
            var ACC_BOWER_PATTERN = process.env.ACC_BOWER_PATTERN;
            var accCustomMatch = false;
            if(ACC_BOWER_PATTERN){
                var pattern = ACC_BOWER_PATTERN.startsWith('/') ? ACC_BOWER_PATTERN : new RegExp(ACC_BOWER_PATTERN);
                accCustomMatch = pattern.test(that._source);
            }
            ...
            index = semver.maxSatisfyingIndex(versionsArr, target, true, accCustomMatch);
            if (index !== -1) {
